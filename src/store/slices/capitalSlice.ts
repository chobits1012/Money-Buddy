    ...
    removePool: (id: string) => {
        set((state) => {
            ...
            const holdings = state.holdings.map((h) =>
                h.poolId === id ? { ...h, poolId: undefined, updatedAt: now } : h,
            );

            if (poolToRemove.type === 'US_STOCK') {
                return { pools, holdings, poolLedger };
            }
